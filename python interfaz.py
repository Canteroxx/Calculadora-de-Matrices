...
# Optional: for modern themes, install ttkbootstrap: pip install ttkbootstrap
try:
...
class MatrixCalculatorApp(tk.Tk):
    ...
    def _render_matrix_entries(self, rA, cA, rB, cB, func):
        ...
        # Use tk.Scrollbar to avoid ttkbootstrap event issue
    ...
    def _setup_unary_input(self, operation, func):
        ...
        def create_table():
            try:
                n = int(size.get())
                if n<1 or n>4: raise ValueError("Tamaño debe ser entre 1 y 4")
            except Exception as e:
                messagebox.showerror("Error", str(e)); return
